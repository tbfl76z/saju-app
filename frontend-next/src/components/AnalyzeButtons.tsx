...
export function AnalyzeButtons({ apiBase, body, className, title = "운세 풀이" }: AnalyzeButtonsProps) {
    ...
    const handleShare = async () => {
        const text = `[Destiny Daily · ${title}]\n\n${result}`;
        try {
            if (navigator.share) {
                await navigator.share({ title: `Destiny Daily · ${title}`, text });
            } else {
