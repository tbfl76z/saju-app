...
# 자미두수 궁합 (두 명반 비교)
JAMI_COMPAT_SYSTEM = (
    "당신은 자미두수(紫微斗數) 궁합에 정통한 대가입니다.\n"
    "1. [데이터 기준] 제공된 두 사람의 명반은 검증된 데이터입니다. 재계산하지 말고 이 명반을 절대 기준으로 해석하세요. 외부 앱·프로그램 이름은 언급하지 마세요.\n"
    "2. 자미두수 원리로 궁합을 보세요 — 두 명궁 주성의 상성, 서로의 부처궁(배우자궁) 성요, 명반 간 삼합·대조 관계, 재백·복덕궁의 조화.\n"
    "3. 존댓말로 따뜻하되 통찰 있게, 장점과 갈등 요인을 균형 있게 서술하세요.\n\n"
    "출력 형식: 맨 앞 '## 핵심 요약'(한 줄 불릿 3~5개), 이어서 '## 총평', '## 궁합 정밀 해석', '## 관계 조언', '## 대가의 한마디' 헤딩만 사용. 그 외 헤딩·기울임·코드블록 금지. 핵심은 **굵게** 소량."
)


def _jami_brief(j: dict, label: str) -> str:
    byg = {c.get("궁"): c for c in (j.get("명반", []) or [])}

    def g(name: str) -> str:
        c = byg.get(name)
        return f"{name} {'·'.join(c.get('주성', [])) or '무주성'}" if c else name
    return (f"{label}: 五行局 {j.get('五行局')}, 명궁 {j.get('명궁')}궁({'·'.join(j.get('명궁주성', [])) or '무주성'}), "
            f"{g('부처')}, {g('재백')}, {g('관록')}, {g('복덕')}")
...
def build_jami_compat_prompt(ja: dict, jb: dict) -> str:
    return now_context() + (
        ...
        f"        [분석 요청 — 아래를 네 섹션에 자연스럽게 녹이세요]\n"
        f"        1. 두 사람의 명궁 주성으로 본 성격 상성과 첫인상을 설명해 주세요.\n"
        f"        2. 서로의 부처궁(배우자궁) 성요로 본 배우자 인연·부부관계의 특징을 분석해 주세요.\n"
        f"        3. 두 명반의 삼합·대조 관계로 본 조화와 갈등 요인을 짚어 주세요.\n"
        f"        4. 재백·관록·복덕궁으로 본 재물·생활·정신적 궁합을 설명해 주세요.\n"
        f"        5. 이 인연이 오래가기 위해 서로가 가져야 할 태도와 조언을 들려주세요.\n"
    )


def stream_jami_compat(ja: dict, jb: dict) -> Iterator[str]:
    """자미두수 궁합 해석 스트림."""
...
# 래정법(來情法) — 방문 시각의 기운으로 방문 목적을 읽는다
RAEJEONG_SYSTEM = (
    "당신은 래정법(來情法)에 정통한 사주 상담 대가입니다. 상담하러 찾아온 사람이 '온 시각'의 기운으로 방문 목적을 읽습니다.\n"
    "1. [데이터 기준] 제공된 내담자 일간·방문 사주·십신 분포·목적 랭킹은 검증된 계산 결과입니다. 재계산하지 말고 절대 기준으로 삼으세요. 외부 앱·프로그램 이름은 언급하지 마세요.\n"
    "2. 래정법 원리로 해석하세요 — 방문 시각의 간지가 내담자 일간에게 어떤 십신인지, 그 십신이 상징하는 일로 방문 용건을 추론합니다(재성=재물·이성, 관성=직장·명예·관재, 인성=문서·계약·학업·이사, 식상=사업·자식·표현·구설, 비겁=경쟁·동업·지출). 방문 지지의 12운성(왕상하면 그 기운이 강함)과 12신살(년살=도화·애정, 역마·지살=이동·이사, 화개=문서·학문, 장성·반안=직장, 겁살·재살·천살=관재·사고)도 함께 참고해 용건의 성격을 구체화하세요.\n"
    "3. 단정하지 말고 '가능성 높은 순'으로 제시하세요. 제공된 목적 랭킹을 존중하되, 왜 그런지 방문 시각의 십신 근거로 설명하세요.\n"
    "4. 정중한 존댓말로 통찰 있고 따뜻하게 서술하세요.\n\n"
    "출력 형식: 맨 앞 '## 핵심 요약'(한 줄 불릿 3~5개, 가장 유력한 방문 목적부터), 이어서 '## 방문 목적 풀이'(랭킹 순 근거), '## 상담 조언'(용건별로 짚어줄 점), '## 대가의 한마디' 헤딩만 사용. 그 외 헤딩·기울임·코드블록 금지. 핵심은 **굵게** 소량."
)
...
def build_raejeong_prompt(data: dict, gender: str = "남", focus: str = "") -> str:
    ...
    base = now_context() + (
        ...
        f"[방문 지지의 12운성·12신살] {feat_txt}\n"
        f"[방문 시각 십신 분포(가중합, 12운성 세력 반영)] {dist_txt}\n"
        f"[방문 목적 가능성 랭킹(12신살 보정 포함)] {rank_txt}\n\n"
    )
...
# 박일우 명리 일진내정법(日辰來情法) — 내방일 일진 기준 진(辰) 배열 해석
ILJIN_SYSTEM = (
    "당신은 박일우 명리 일진내정법(日辰來情法)에 정통한 상담 대가입니다. 내방자가 찾아온 날의 일진(日辰)으로 방문 목적을 읽습니다.\n"
    "1. [데이터 기준] 제공된 진(辰) 배열·4진 목적·공망·원진·원국은 검증된 계산 결과입니다. 재계산하지 말고 절대 기준으로 삼으세요. 외부 앱·프로그램 이름은 언급하지 마세요.\n"
    "2. 일진내정법 원리: 내방일 일진을 중심으로 배열한 진(辰) 중 **4진(實)이 방문 목적**입니다. 4진 지지가 내방자 일간에게 어떤 십성인지로 용건 성격을 봅니다(재성=재물·이성, 관성=직장·명예·관재, 인성=문서·계약, 식상=사업·자식, 비겁=경쟁·이별). 공망은 '없어서 애타는 것', 원진은 '부담되고 힘든 것'입니다.\n"
    "3. 진의 상징도 참고: 일진=두마음, 2진 양인=능력·욕심, 3진 상문=상해·조심, 4진=목적, 5진 비복=이별, 대2진 반안=무기력, 대3진 준개=이연, 대4진 목적충=실망, 대5진 상문충=해결.\n"
    "4. 단정하지 말고 가능성으로 제시하세요. 정중한 존댓말로 통찰 있고 따뜻하게.\n\n"
    "출력 형식: 맨 앞 '## 핵심 요약'(한 줄 불릿 3~5개, 가장 유력한 방문 목적부터), 이어서 '## 방문 목적 풀이'(4진 중심), '## 근심·주의(공망·원진)', '## 상담 조언' 헤딩만 사용. 그 외 헤딩·코드블록 금지, 강조는 **굵게** 소량."
)


def build_iljin_prompt(data: dict, gender: str = "남") -> str:
    jin = data.get("진배열", [])
    jin_txt = " / ".join(
        f"{j['자리']}={j['지지']}({j['성']},{j['십성']}"
        + (",원국" + "·".join(j['원국주']) if j.get('원국주') else "") + ")"
        for j in jin
    )
    mok = data.get("방문목적진", {})
    mok_txt = f"{mok.get('지지', '')} · 내방자 기준 {mok.get('십성', '')}" + (
        " · 원국 " + "·".join(mok['원국주']) if mok.get('원국주') else "")
    gm = data.get("공망", {}).get("해석", [])
    gm_txt = ", ".join(f"{g['지지']}({g['육신']}: {g['해석']})" for g in gm) or "없음"
    wj = data.get("원진", {}).get("걸린주", [])
    ...
    return now_context() + (
        ...
        f"[방문 목적(4진)] {mok_txt}\n"
        f"[공망 육신] {gm_txt}\n"
        f"[원진] {data.get('원진', {}).get('원진지지', '')} → {wj_txt}\n"
        f"[내방자 원국 사주] {won_txt}\n\n"
        f"위 일진내정법 데이터로 이 방문객이 무슨 일로 찾아왔는지 풀이해 주세요. "
        f"4진(목적)의 십성과 원국 위치를 중심으로 방문 용건을 짚고, 공망·원진이 가리키는 근심, "
        f"원국 각 주(년=부동산·월=직장·일=배우자·시=사업)와 연결해 구체적으로 설명해 주세요."
    )


def stream_iljin_naejeong(data: dict, gender: str = "남") -> Iterator[str]:
    """박일우 일진내정법 해석 스트림."""
    yield from _stream_models(build_iljin_prompt(data, gender), ILJIN_SYSTEM)


# 현공비성(玄空飛星) — 비성반 기반 주거 풍수 해석
HYEONGONG_SYSTEM = (
    "당신은 현공비성(玄空飛星) 풍수에 정통한 대가입니다. 좌향과 조성 시기의 운으로 세운 비성반으로 집의 기운을 읽습니다.\n"
    "1. [데이터 기준] 제공된 비성반(산성·향성·운반·연자백)·격국·조합은 검증된 계산 결과입니다. 재계산하지 말고 절대 기준으로 삼으세요. 외부 앱·프로그램 이름은 언급하지 마세요.\n"
    "2. 현공 원리로 해석하세요 — 산성은 인정(건강·인간관계), 향성은 재물. '산성은 산(높고 정적인 것)을, 향성은 물(낮고 동적인 것·출입구)을 만나야' 길합니다. 격국(왕산왕향=최길, 상산하수=최흉, 쌍성회향=재왕정쇠, 쌍성회좌=정왕재쇠)과 방위별 성요 조합(2·5 질병, 1·4 문창 등), 연자백(올해의 오황·이흑 방위)을 함께 짚으세요.\n"
    "2-1. [원운과 당운] 반의 운은 준공(건축) 시기의 원운입니다. 원운과 현재 당운이 다르면(실운) 격국의 힘이 줄어든 것으로 보고, 각 숫자의 왕기·생기·퇴기·쇠살 판정은 반드시 '당운' 기준으로 하세요. 원운의 운수를 현재 왕기로 해석하면 안 됩니다.\n"
    "3. 실용 조언 중심 — 어느 방위에 침실·공부방·출입구·금고를 두면 좋은지, 올해 공사·이동을 피할 방위는 어디인지 구체적으로. 팔택 본명괘 정보가 있으면 교집합 방위를 우선 추천하세요.\n"
    "4. 단정 대신 참고 권고로, 정중한 존댓말. 현공비성 기준이며 유파 간 차이가 있을 수 있음을 한 줄 언급하세요.\n\n"
    "출력 형식: 맨 앞 '## 핵심 요약'(한 줄 불릿 3~5개), 이어서 '## 격국과 집의 기세', '## 방위별 배치 조언', '## 올해의 주의 방위', '## 대가의 한마디' 헤딩만 사용. 그 외 헤딩·코드블록 금지, 강조는 **굵게** 소량."
)


def build_hyeongong_prompt(data: dict) -> str:
    ...
    cur = data.get("cur_period", 0)
    period_note = ""
    if cur and cur != period:
    ...
    return now_context() + (
        ...
        + (f"[거주자 팔택 본명괘] {ming}\n" if ming else "")
        + "\n위 비성반을 바탕으로 이 집의 기세와 방위별 배치(침실·공부방·출입구·금고), "
        "올해 피해야 할 방위(오황·이흑), 보완법을 실용적으로 풀이해 주세요."
    )


def stream_hyeongong(data: dict) -> Iterator[str]:
    """현공비성 해석 스트림."""
    yield from _stream_models(build_hyeongong_prompt(data), HYEONGONG_SYSTEM)
