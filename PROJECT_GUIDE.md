...
- **방법**:
  1. `data/` 폴더에 원하는 MD 또는 TXT 파일들을 넣습니다. 추가 학습 자료는 `data/학습자료/`에 모읍니다.
  2. PDF 원본은 먼저 Markdown으로 변환합니다: `python convert_pdfs_to_md.py "원본PDF폴더" "../data/학습자료"`
  3. 다음 스크립트를 실행합니다: `python extract_knowledge.py`
  4. `knowledge_base.txt`가 생성되며 AI 분석 컨텍스트에 즉시 반영됩니다.
